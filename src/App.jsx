...
import { Alert, Button, Card, Col, ConfigProvider, Empty, Flex, Form, InputNumber, Progress, Row, Select, Space, Spin, Statistic, Tag, Typography, message } from 'antd';
...
export default function App() {
  ...
  useEffect(() => {
    const timeout = window.setTimeout(async () => {
      await Promise.all([refreshPrices(), new Promise((resolve) => window.setTimeout(resolve, 2000))]);
  ...
  const addPosition = (values) => {
    ...
    setPositions((current) => [{ id: `${Date.now()}-${values.coinId}`, coinId: values.coinId, symbol: coin.symbol, name: coin.name, amount: values.amount, purchasePrice: values.purchasePrice }, ...current]);
    form.resetFields();
  ...
  const allocation = useMemo(() => {
    const grouped = positions.reduce((result, position) => {
      const value = position.amount * (prices[position.coinId]?.usd ?? position.purchasePrice);
      result[position.coinId] = (result[position.coinId] || 0) + value;
      return result;
    }, {});
    return Object.entries(grouped).map(([coinId, value]) => ({ coin: coins.find((item) => item.id === coinId), value, share: currentTotal ? (value / currentTotal) * 100 : 0 })).sort((a, b) => b.value - a.value);
  }, [positions, prices, currentTotal]);

  if (initialLoading) return <ConfigProvider theme={{ token: { colorPrimary: '#2563eb', borderRadius: 10, fontFamily: 'Inter, Arial, sans-serif' } }}><div className="loading-screen"><Spin size="large" /><Typography.Text type="secondary">Загружаем актуальные котировки</Typography.Text></div></ConfigProvider>;

  return <ConfigProvider theme={{ token: { colorPrimary: '#2563eb', borderRadius: 10, fontFamily: 'Inter, Arial, sans-serif' } }}><div className="app"><header className="header"><div className="container header-inner"><Flex align="center" gap={10}><div className="logo">C</div><Typography.Text strong className="brand">crypto-app</Typography.Text></Flex></div></header>
    <main className="container main"><section className="intro"><Typography.Title level={1}>Сколько стоит ваша покупка сейчас?</Typography.Title><Typography.Paragraph type="secondary">Укажите данные покупки. Мы сравним их с актуальной ценой и покажем результат.</Typography.Paragraph></section>
      ...
      <Card className="form-card"><Form form={form} layout="vertical" onFinish={addPosition} requiredMark={false}><Row gutter={[16, 0]} align="bottom"><Col xs={24} md={12} lg={8}><Form.Item label="Валюта" name="coinId" rules={[{ required: true, message: 'Выберите валюту' }]}><Select showSearch placeholder="Например, Bitcoin" onChange={selectCoin} optionFilterProp="searchText" options={coins.map((coin) => ({ value: coin.id, label: <CoinOption coin={coin} />, searchText: `${coin.name} ${coin.symbol}` }))} /></Form.Item></Col><Col xs={24} sm={12} lg={6}><Form.Item label="Количество" name="amount" rules={[{ required: true, message: 'Укажите количество' }]}><InputNumber min={0.00000001} precision={8} placeholder="0.00" /></Form.Item></Col><Col xs={24} sm={12} lg={6}><Form.Item label="Цена покупки" name="purchasePrice" rules={[{ required: true, message: 'Укажите цену' }]}><InputNumber min={0.00000001} precision={8} prefix="$" placeholder="0.00" /></Form.Item></Col><Col xs={24} md={12} lg={4}><Form.Item><Button type="primary" htmlType="submit" icon={<PlusOutlined />} block>Добавить</Button></Form.Item></Col></Row></Form></Card>
      ...
      {allocation.length > 0 && <AllocationChart items={allocation} total={totals.current} />}
    </main></div></ConfigProvider>;
}

function CoinOption({ coin }) { return <Space><img className="coin-icon small" src={coin.icon} alt="" /><span>{coin.name}</span><Typography.Text type="secondary">{coin.symbol}</Typography.Text></Space>; }
...
function AllocationChart({ items, total }) {
  const colors = ['#2563eb', '#16a36f', '#f59e0b', '#e14d64', '#7c3aed', '#0891b2', '#64748b'];
  const segments = items.map((item, index) => { const start = items.slice(0, index).reduce((sum, entry) => sum + entry.share, 0); return `${colors[index % colors.length]} ${start}% ${start + item.share}%`; }).join(', ');
  return <section className="allocation"><Flex justify="space-between" align="center"><Typography.Title level={2}>Распределение кошелька</Typography.Title><Typography.Text type="secondary">{formatMoney(total)}</Typography.Text></Flex><Card><Row gutter={[28, 24]} align="middle"><Col xs={24} md={10}><div className="pie-chart" style={{ background: `conic-gradient(${segments})` }}><div className="pie-center"><Typography.Text type="secondary">Всего</Typography.Text><strong>{formatMoney(total)}</strong></div></div></Col><Col xs={24} md={14}><div className="allocation-legend">{items.map((item, index) => <div className="legend-item" key={item.coin.id}><Flex align="center" gap={10}><span className="legend-color" style={{ background: colors[index % colors.length] }} /><img className="coin-icon small" src={item.coin.icon} alt="" /><Typography.Text strong>{item.coin.name}</Typography.Text></Flex><Flex align="center" gap={10}><Typography.Text type="secondary">{formatMoney(item.value)}</Typography.Text><Tag>{item.share.toFixed(1)}%</Tag></Flex></div>)}</div></Col></Row></Card></section>;
}
